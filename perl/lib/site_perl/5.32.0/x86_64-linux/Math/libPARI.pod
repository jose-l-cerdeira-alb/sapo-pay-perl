...
arguments C<x> are allowed in the present version B<2.3.5>. If C<I<flag>> is equal to
...
B<2.3.5>, this is implemented only for elliptic curves defined over B<I<Q>>.
...
polynomial C<x belongs to B<I<Q>>[X]>. In the present version B<2.3.5>, C<x> must be irreducible
